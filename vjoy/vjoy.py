#!/usr/bin/env python
# sudo pip install https://github.com/gvalkov/python-evdev/archive/master.zip
# https://github.com/gvalkov/python-evdev/blob/master/doc/index.rst
...
keyboards = [
	[('EV_LED', 17L), ('EV_MSC', 4L), ('EV_KEY', 1L), ('EV_SYN', 0L)]
]
mice = [
	[('EV_KEY', 1L), ('EV_REL', 2L), ('EV_SYN', 0L), ('EV_MSC', 4L)],
	[('EV_KEY', 1L), ('EV_REL', 2L), ('EV_SYN', 0L)]
]
...
# Get keyboard device.
if args.keyboard == None:
	for keyboard in keyboards:
		dev_kb = get_device(keyboard)
		if dev_kb is not None:
			break
else:
	dev_kb = InputDevice(args.keyboard)
...
if args.mouse == None:
	for mouse in mice:
		dev_mouse = get_device(mouse)
		if dev_mouse is not None:
			break
else:
	dev_mouse = InputDevice(args.mouse)
