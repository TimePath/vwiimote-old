...
##     \/__/         \/__/         \/__/             
##
##     WMD v0.0.3.99999: Wiiwanna Motte Dammiit!
##     by Francois A. Bradet, aka e-Hernick#wiili@freenode
##     coded from 2006-12-04 to 2006-12-08 19:43 EDT
##     with code by Micah Dowty and Marcan
##     with algorithms by Ian Rickard
##     share use modify as you wish; CC-PD+GPL2
##  _    _   _____                  _   _                                
## | |  | | |  __ \                | | | |                               
## | |  | | | |__) |   ___  _ __   | |_| |__   ___  __      ____ _ _   _ 
## | |  | | |  _  /   / _ \| '_ \  | __| '_ \ / _ \ \ \ /\ / / _` | | | |
## | |__| | | | \ \  | (_) | | | | | |_| | | |  __/  \ V  V / (_| | |_| |
##  \____/  |_|  \_\  \___/|_| |_|  \__|_| |_|\___|   \_/\_/ \__,_|\__, |
##                                                                 __/ |
##                                                                |___/ 
##  _              _           _                   _   _             _ 
## | |            | |         | |                 | | (_)           | |
## | |_ ___     __| | ___  ___| |_ _ __ _   _  ___| |_ _  ___  _ __ | |
## | __/ _ \   / _` |/ _ \/ __| __| '__| | | |/ __| __| |/ _ \| '_ \| |
## | || (_) | | (_| |  __/\__ \ |_| |  | |_| | (__| |_| | (_) | | | |_|
##  \__\___/   \__,_|\___||___/\__|_|   \__,_|\___|\__|_|\___/|_| |_(_)
##
############################################################
#######  I. INTRODUCTION:   READ ME FIRST OR ELSE  #########
##**********************************************************
## It is important that you read this section before
...
## WMD lets you use the Wiimote as a joystick
...
##
##      To use uinput events to move the mouse (OK)
##  ++  Xorg 7.0 with evdev
##
############################################################
...
def send_command_code(cc):
  ...
  if name == "led" and cc[1] == "toggle":
    ...
    osd_show( "**HOME Menu** [Back] GuessOrientation ExitWMD" )
    status['go']= 1
...
class IR_Pointer:
  ...
  def preprocess_dots( self, dots ):
	...
	dots[dn][0] = int(x)
	dots[dn][1] = int(y)
	log(DEBUG_IR, "dn = %u, lx = %u, ly = %u, mx = %u, my = %u, s = %u, x = %u, y = %u" % (dn, lx, ly, mx, my, s, x, y))

    if dots_sum < 255*6 and dots_on == 2:
      x1 = dots[0][0]
      y1 = dots[0][1]
      x2 = dots[1][0]
      y2 = dots[1][1]
  ...
  def process_dots( self, dots ):
    dots = self.preprocess_dots(dots)

    if len(dots_fifo)+1 > dots_fifo_len:
      dots_fifo.pop(0)

    if dots:
      dots_fifo.append(dots)
    else:
      return 0

    dots_fifo_sum = [[0,0],[0,0]]
    dots_fifo_avg = [[0,0],[0,0]]
    dots_sum_dn = [0,0]
    dots_sum = 0

    if len(dots_fifo) == dots_fifo_len:
      if dots_quash['age'] == 0:
	for dp in dots_fifo:
	  for dn in range(2):
	    dn_sum = 0
	    for dax in range(2):
	      dotc_val = dp[dn][dax]
	      dots_fifo_sum[dn][dax] += dotc_val
	      dots_sum += dotc_val
	      dn_sum += dotc_val
	    dots_sum_dn[dn] += dn_sum
	for dn in range(2):
	  for dax in range(2):
	    dots_fifo_avg[dn][dax] = dots_fifo_sum[dn][dax] / dots_fifo_len

	x1 = dots_fifo_avg[0][0]
	y1 = dots_fifo_avg[0][1]

	x2 = dots_fifo_avg[1][0]
	y2 = dots_fifo_avg[1][1]

	log(DEBUG_IR, "Got two lit dots: %u,%u : %u,%u" % (x1, y1, x2, y2))

	self.process_abs_report(x1, x2, y1, y2)
	      
	dots_quash['age'] = dots_quash['duration']
      else:
	dots_quash['age'] -= 1

  def process_abs_report( self, x1, x2, y1, y2 ):
    if IO_MODES['XLIB'] and IO_CHANNELS['EV_ABS'] == 'XLIB':
      abs_xlib = self.abs_report_processor(x1, x2, y1, y2, XL['w'], XL['h'], 1, 0)
    ...
    if IO_MODES['UINPUT'] and IO_CHANNELS['EV_ABS'] == 'UINPUT':
      ...
      abs_uinput = self.abs_report_processor(x1, x2, y1, y2, xscale, yscale, 0, 1)
      uinput_abs_report( *abs_uinput )

  def abs_report_processor( self, x1, x2, y1, y2, xscale, yscale, xinv, yinv ):
    xrange = 1024
    yrange = 768
    
    sx = self.abs_scale_axis( x1, x2, xscale, xinv, xrange, XDZ )
    sy = self.abs_scale_axis( y1, y2, yscale, yinv, yrange, YDZ )
  ...
  def abs_smoothen( self, x, y ):
    ...
    for thresh in threshes:
      ...
      if d < dMin:
        Wei1 = thresh[1]
	Wei2 = SS1 / ( SS2 + d*d*d* SS3 )

        Wei = ( Wei1 * SAW ) + (Wei2 * ( 1-SAW ) )
        sx = ( self.screenx * Wei ) + ( x * (1-Wei) )
	sy = ( self.screeny * Wei  ) + ( y * (1-Wei) )
	print "%s %s %s" % (Wei1, Wei2, Wei)
	break
    ...
    if (1):
      ...
      log(LOG_IR, "d=%d, x=%d, y=%d, od=%d sx=%d, sy=%d" % (d, x, y, od, sx, sy) )

    return [sx, sy]

  def abs_scale_axis( self, p1, p2, scale, inv, range, dz ):
    avgp = ( p1 + p2 ) / 2

    sp = avgp * ( ( scale / (1-dz) ) / range )
    sp -= (scale * dz)
